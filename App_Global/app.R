...
server <- function(input, output) {
  ...
  output$slovenia <- renderPlot({
    ...
    datos_slovenia_g <- datos_no_na %>% filter(country == 'Slovenia')
    datos_national <- datosNational %>% filter(Year %in% c(2009,2010,2011,2012,2013,2014,2015,2016,2017,2018)) %>% filter(Country=='Slovenia')
    datos_national <- select(datos_national,Country,Value,Year)
    datos_slovenia_g <- cbind(datos_slovenia_g,datos_national$Value)
    
    ggplot(datos_slovenia_g,aes(x=as.numeric(year))) + geom_line(aes(y=as.numeric(global_index),color='global_index')) + 
